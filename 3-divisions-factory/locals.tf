locals {
  ...
  trusted_ppd_hub    = local.division_config.environments.ppd.shared_services.networking.trusted   # todo: handle usecase whereas there is no networking for ppd
  untrusted_ppd_hub  = local.division_config.environments.ppd.shared_services.networking.untrusted # todo: hande usecase whereas there is no networking for ppd
}
